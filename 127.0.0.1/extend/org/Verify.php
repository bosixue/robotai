...
class Verify
{
    ...
    public function check($code, $id = '')
    {
        ...
        // 验证码不能为空
        $secode = session($key);
        $secode = isset($secode)?$secode:null;
    ...
    {
        // 图片宽(px)
        $this->imageW || $this->imageW = $this->length * $this->fontSize * 1.5 + $this->length * $this->fontSize / 10;
        // 图片高(px)
        ...
        if ($this->useZh) {
            ...
            for ($i = 0; $i < $this->length; $i++) {
                ...
                imagettftext($this->_image, $this->fontSize, mt_rand(-10, 10), $codeNX, $this->fontSize * 1.6, $this->_color, $this->fontttf, $code[$i]);
            }
        }
        ...
        session($key . $id, $secode);
				...
				// $verify_code_token = md5(time());
				// setcookie('verify_code_token', $verify_code_token, time() + (3600 * 24), '/', '.at58.com');
				// if(isset($_GET['domain']) === true){
				// 	$domain = $_GET['domain'];
				// 	$a = setcookie('verify_code_token', $verify_code_token, time() + (3600 * 24), '/', 'ai.sc-yun.com', false);
				// }else{
				// // 	setcookie('verify_code_token', $verify_code_token, time() + (3600 * 24), '/');
				// }
				// $redis->setex('register_verify_code_'.$verify_code_token, (3600 * 24), $verify_code);
