...
class AdminData extends Base{
  ...
  */
   ...
   * @return string
  */
  public function get_role_id($member_id)
  {
  	if(empty($member_id)){
  		return false;
  	}
  ...
  /**
	...
	*/
	public function get_find_distribution_robot_count($member_id)
	{
		...
		$count = Db::name('robot_distribution_record')
							->where([
								'pid'	=>	$member_id,
								'state'	=>	1
							])
	...
	* @return int 
	*/
	public function get_number_numbers($member_id){
		if(empty($member_id)){
			return false;
		}
		//->where('mobile', 'is not null')
		//这种写法貌似不起作用 改了一下 如有问题自行恢复即可
		$numbers_count = Db::name('member')
									->alias('m')
									->join('tel_config tc', 'tc.task_id = m.task', 'LEFT')
	...
	* @return int 
	*/
	public function get_calls_numbers($member_id){
		if(empty($member_id)){
			return false;
		}
		$calls_count = Db::name('member')
									->alias('m')
									->join('tel_config tc', 'tc.task_id = m.task', 'LEFT')
									->where('m.owner', $member_id)
									->where('m.status', '>=', 2)
									->where('tc.status', '>', 0)
									->count('*');
		\think\Log::record('获取用户的已呼叫号码总数11');
		if(empty($calls_count)){
			return 0;
		}else {
			return $calls_count;
		}
	...
	* @return int 
	*/
	public function get_connected_numbers($member_id){
		if(empty($member_id)){
			return false;
		}
		$connected_count = Db::name('member')
									->alias('m')
									->join('tel_config tc', 'tc.task_id = m.task', 'LEFT')
									->where('m.owner', $member_id)
									->where('m.status', 2)
	...
	* @return int 
	*/
	public function get_intentional_customers($member_id){
		if(empty($member_id)){
			return false;
		}
		$intentional_customers = Db::name('member')
									->alias('m')
									->join('tel_config tc', 'tc.task_id = m.task', 'LEFT')
									->where('m.owner', $member_id)
									->where('m.level', '>', 4)
									->where('tc.status','>', 0)
									->count();
		if(empty($intentional_customers)){
			return 0;
		}else {
			return $intentional_customers;
		}
	}
