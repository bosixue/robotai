...
(function($) {
	...
	/* 基础对象检测 */
	RuiKeCMS || $.error("RuiKeCMS基础配置没有正确加载！");
	 ...
	 */
	RuiKeCMS.parse_url = function(url) {
	...
	RuiKeCMS.parse_str = function(str) {
		var value = str.split("&"),
			vars = {},
			param;
		for (val in value) {
	...
	RuiKeCMS.parse_name = function(name, type) {
		if (type) {
			/* 下划线转驼峰 */
	...
	RuiKeCMS.U = function(url, vars, suffix) {
		...
		if (0 === url.indexOf("/")) { //路由模式
			...
			path[1] || $.error("RuiKeCMS.U(" + url + ")没有指定控制器");
	...
	/* 设置表单的值 */
	RuiKeCMS.setValue = function(name, value) {
