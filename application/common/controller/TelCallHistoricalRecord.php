...
//通话记录处理类
class TelCallHistoricalRecord extends Base{
    ...
    public function screent($args, $page, $limit){
        ...
        $tablename = get_table_name($args['select_type']);
...
//			$where['tcr.level'] = ['=', $args['level']]
        ...
        //排重(四个字段可以排除重复)
        //$condition = "tcr.id = (select min(`A`.id) from rk_$table as A where  `tcr`.mobile=`A`.mobile and `tcr`.task_id = `A`.task_id and `tcr`.scenarios_id = `A`.scenarios_id and `tcr`.last_dial_time = `A`.last_dial_time)";
    ...
    public function get_id($mobile, $task_id)
    {
        ...
        $id = Db::name('tel_call_record_history')
            ->where([
                'mobile'	=>	$mobile,
                'task_id'	=>	$task_id
            ])
