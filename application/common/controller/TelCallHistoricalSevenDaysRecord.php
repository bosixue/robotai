...
//通话记录处理类
class TelCallHistoricalSevenDaysRecord extends Base{
    ...
    public function screent($args, $page, $limit){
        ...
        $user_auth = session('user_auth');
        $connect_where = [];
        $table = Db::name('tel_call_record_sevendays_history')
            ->field('tcr.id,tcr.mobile,tcr.duration,tcr.status,tcr.level,tcr.last_dial_time,tcr.task_id,state_crm,tcr.scenarios_id,tcr.review')
            ->alias('tcr');
        //->join('tel_scenarios ts', 'tcr.scenarios_id = ts.id', 'LEFT')
        //->join('tel_config tc', 'tc.task_id = tcr.task_id', 'LEFT');
        $count = Db::name('tel_call_record_sevendays_history')
            ->alias('tcr');
        $where = [];
        $connect_where = [];
...
//			$where['tcr.level'] = ['=', $args['level']]
        ...
        if(isset($args['call_times']) === true && $args['call_times'] != ''){
            ...
            $whereTalkTime[]=  ['tcr.call_times'=>['>=', $args['call_times']]];
        }
        //有效对话次数
        if(isset($args['effective_times']) === true && $args['effective_times'] != ''){
            $whereTalkTime[]= ['tcr.effective_times'=>['>=', $args['effective_times']]];
        }
        //触发问题次数
        if(isset($args['hit_times']) === true && $args['hit_times'] != ''){
            $whereTalkTime[]=   ['tcr.hit_times'=>['>=', $args['hit_times']] ];
        }

        $whereAttitude=[];
        //肯定次数
        if(isset($args['affirm_times']) === true && $args['affirm_times'] != ''){
            $whereAttitude[]= [ 'tcr.affirm_times'=>[ '>=', $args['affirm_times']  ]];
        }
        //中性次数
        if(isset($args['neutral_times']) === true && $args['neutral_times'] != ''){
            $whereAttitude[]= [ 'tcr.neutral_times'=>[ '>=', $args['neutral_times']  ]];
        }
        //否定次数
        if(isset($args['negative_times']) === true && $args['negative_times'] != ''){
            $whereAttitude[]= [ 'tcr.negative_times'=>[ '>=', $args['negative_times'] ] ];
        }
        //是否邀约成功
        ...
        //排重(四个字段可以排除重复)
        $condition = "tcr.id = (select min(`A`.id) from rk_tel_call_record_sevendays_history as A where  `tcr`.mobile=`A`.mobile and `tcr`.task_id = `A`.task_id and `tcr`.scenarios_id = `A`.scenarios_id and `tcr`.last_dial_time = `A`.last_dial_time)";
		...
		*/
            ...
            if(isset($args['type']) === true && !empty($args['type'])){
                ...
                if(isset($args['order']) === true && !empty($args['order'])){
                    if($args['type'] == "duration"){
                        $table_ = $table->page($page, $limit)->order(['duration'=>$args['order']])->select();
                    }elseif($args['type']=="call_time"){
                        $table_ = $table->page($page, $limit)->order(['last_dial_time'=>$args['order']])->select();                    }
                }else{
                    //第一次进来按照最后拨打时间排序
                    $table_ = $table->page($page, $limit)->order(['last_dial_time'=>'desc'])->select();
                }
            }else{
                //第一次进来按照最后拨打时间排序
                $table_ = $table->page($page, $limit)->order(['last_dial_time'=>'desc'])->select();
             ...
             //page=0 代表 全部 很重要  导出全部  
            //筛选排序类型 type  duration通话时长  or call_time 最后拨打时间
            if(isset($args['type']) === true && !empty($args['type'])){
                //筛选排序 方式 desc or asc
                if(isset($args['order']) === true && !empty($args['order'])){
                    if($args['type']=="duration"){
                        $table_ = $table->order(['duration'=>$args['order']])->select();
                    }elseif($args['type']=="call_time"){
                        $table_ = $table->order(['last_dial_time'=>$args['order']])->select();
                    }
                }else{
                    //第一次进来按照最后拨打时间排序
                    $table_ = $table->order(['last_dial_time'=>'desc'])->select();
                }
            }else{
                //第一次进来按照最后拨打时间排序
                $table_ = $table->order(['last_dial_time'=>'desc'])->select();
        ...
        $count = $count->distinct('`tcr`.mobile,`tcr`.task_id,`tcr`.scenarios_id,`tcr`.last_dial_time')->count(1);
        //查询任务表
    ...
    public function get_id($mobile, $task_id)
    {
        ...
        $id = Db::name('tel_call_record_sevendays_history')
            ->where([
                'mobile'	=>	$mobile,
                'task_id'	=>	$task_id
            ])
